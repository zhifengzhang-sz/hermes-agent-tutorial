...
Concretely: you chat with Hermes from Telegram, Hermes plans the work, and when the work is coding-heavy it spawns `claude -p "..."` as a subprocess to do the actual implementation. **The subprocess bills against your Claude subscription, not Extra usage** — confirmed via `claude auth status` showing `apiProvider: "firstParty"` plus a live delegation test on 2026-04-18 (see [llm-provider-scenarios.md § Scenario 3](llm-provider-scenarios.md#scenario-3-openrouter--claude-code-delegation) for the mechanism and [REVIEW.md](../REVIEW.md) for the evidence). Hermes itself is then best run on a cheap non-Anthropic model via OpenRouter or similar, since orchestration work usually doesn't need frontier-tier reasoning.
