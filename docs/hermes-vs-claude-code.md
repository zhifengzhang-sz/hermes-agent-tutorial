...
Concretely: you chat with Hermes from Telegram, Hermes plans the work, and when the work is coding-heavy it spawns `claude -p "..."` as a subprocess to do the actual implementation. **The subprocess bills against your Claude subscription, not Extra usage** — the primary evidence is `claude auth status` showing `apiProvider: "firstParty"` (Anthropic's server-side designation for subscription-backed routing), confirmed live on 2026-04-18. A corroborating delegation test ran end-to-end successfully but used too few tokens to independently distinguish routing buckets on its own — see [llm-provider-scenarios.md § Scenario 3](llm-provider-scenarios.md#scenario-3-openrouter--claude-code-delegation) for the mechanism and [REVIEW.md](../REVIEW.md) for the full evidence chain. Hermes itself is then best run on a cheap non-Anthropic model via OpenRouter or similar, since orchestration work usually doesn't need frontier-tier reasoning.
