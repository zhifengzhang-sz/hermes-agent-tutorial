...
A common setup given the above: run Hermes on a non-Anthropic model via OpenRouter or OpenAI (cheaper per turn for the bookkeeping-style work Hermes does), and let Hermes **delegate heavy coding tasks to Claude Code** as a sub-agent ([chapter 8](08-claude-code-integration.md)). The Claude Code subprocess spawned by Hermes bills against your subscription (not Extra usage) — `claude auth status` shows `apiProvider: "firstParty"`, which is Anthropic's server-side designation for subscription-backed routing. That `apiProvider` signal was confirmed live on 2026-04-18 (it's the primary evidence; the corroborating delegation test used too few tokens to independently distinguish routing buckets on its own); see [llm-provider-scenarios.md § Scenario 3](llm-provider-scenarios.md#scenario-3-openrouter--claude-code-delegation) for the full mechanism.
