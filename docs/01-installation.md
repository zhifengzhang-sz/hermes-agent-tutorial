...
A common setup given the above: run Hermes on a non-Anthropic model via OpenRouter or OpenAI (cheaper per turn for the bookkeeping-style work Hermes does), and let Hermes **delegate heavy coding tasks to Claude Code** as a sub-agent ([chapter 8](08-claude-code-integration.md)). The expectation is that the Claude Code subprocess spawned by Hermes is billed the same way as a `claude` invocation you'd type yourself — i.e., against your subscription — because from Anthropic's API perspective it's the same binary making the call. This tutorial hasn't verified that empirically against the third-party-app Extra-usage wall above; before committing to this setup, run one subprocess call and check claude.ai/settings/usage to confirm.
