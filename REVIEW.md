...
| Hermes ships ~75 bundled skills out of the box, across ~20 categories (autonomous-ai-agents, creative, data-science, devops, email, gaming, github, mlops, productivity, research, etc.) | [hermes-vs-claude-code](docs/hermes-vs-claude-code.md) | Live-observed 2026-04-18 via `hermes` startup banner on a fresh install: "28 tools · 75 skills". Exact count may drift with Hermes updates |
| Hermes supports ~14 messaging-gateway platforms out of the box: Telegram, Discord, Slack, WhatsApp, Signal, Email, SMS, Home Assistant, plus specialized platforms DingTalk, Feishu, WeCom (+ Callback), Weixin, BlueBubbles, QQBot. Each ships as its own pip extra | [06](docs/06-messaging-gateways.md), [hermes-vs-claude-code](docs/hermes-vs-claude-code.md) | Live-observed 2026-04-18 via `hermes status`'s "Messaging Platforms" section; extras verified in `pyproject.toml` (`sms`, `dingtalk`, `feishu`, `slack`, etc.) |
