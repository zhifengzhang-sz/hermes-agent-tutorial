...
| **Claude Code subprocess delegation bills against the user's Anthropic subscription, not Extra usage.** The `claude` CLI identifies itself as `apiProvider: "firstParty"` to Anthropic's API (visible via `claude auth status`), which is the server-side designation that routes calls against subscription quota. By contrast, Hermes calling the Anthropic API directly with the same OAuth token has no first-party identification and routes to the Extra-usage wall (HTTP 400 if unfunded). Hermes spawning `claude -p "..."` via its terminal tool therefore hits subscription, not Extra usage | [llm-provider-scenarios](docs/llm-provider-scenarios.md), [hermes-vs-claude-code](docs/hermes-vs-claude-code.md), [08](docs/08-claude-code-integration.md) | `claude auth status` output on 2026-04-18 showed `apiProvider: "firstParty"`; live delegation test with Qwen3-235B orchestrator → `claude -p` subprocess returned `391` in ~12s end-to-end; subscription usage at 14%/Extra usage at $0 before and after (consistent with a too-small-to-register subscription call, not with hitting Extra-usage wall which would have returned HTTP 400) |
