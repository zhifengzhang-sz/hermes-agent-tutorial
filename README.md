...
> **Verification status (2026-04-17):** structural claims (file paths, config schemas, auto-loaded files, skill discovery) have been cross-checked against the [Hermes source tree](https://github.com/NousResearch/hermes-agent) and cited where they appear. Runtime-behavior claims (token budgets, `/compress` internals, the auto-skill cadence) have **not** been validated on a clean-VM install yet — treat those as community-sourced ballparks. The full ledger of what's verified vs. pending lives in [`REVIEW.md`](REVIEW.md).
